...
def _extract_text_from_pdf(file_bytes: bytes) -> str:
    ...
    Raises RuntimeError with a specific reason if both the direct extraction
    and the OCR fallback fail to produce any text, so the caller can show the
    real cause instead of a generic "no text found" message.
    """
    text_parts = []
    text_error = None
    try:
